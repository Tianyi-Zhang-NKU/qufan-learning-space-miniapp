...
- 身份：一个手机号可拥有学生/家长、教师和管理员身份；登录时选择身份，个人页可随时切换。
- 教研：浏览器端关联 Word/PDF 原始资料，管理可复用资料包、题目单元、版本发布和课次绑定；一个已发布资料包可供平行班复用，修订后保留旧绑定版本。
...
小程序项目根目录可直接用微信开发者工具导入。2026-07-13 已在本机通过微信开发者工具 CLI 打开项目并成功生成预览构建。该结果仅证明 CLI 可用与预览构建成功，不等同于完整的模拟器流程验收，也不等同于真机验收。
